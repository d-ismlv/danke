...
/**
 * The server half of a study session: it builds the queue, so `ts-fsrs` never
...
export default async function StudyScreen({
  ...
  scope: Scope;
  /** What is being studied, for the end of the session. */
  what: string;
  backHref: string;
  backLabel: string;
  round: number;
}) {
  const { cards, total } = await buildQueue(scope, now());
